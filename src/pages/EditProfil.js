...
import { getProfilUser,editProfilUser} from "../methods/AuthMethod";
...
export default function EditProfilUser(props) {
    ...
    const getUserDetail = async (token) => {
        try{
        const res = await getProfilUser(token);
        if(res.status === 200){
    ...
    useEffect(() => {
      ...
      const handleProfilUser = async (token,id,firstName,lastName,email,password,passwordConfirm) => {
        try{
            const res = await   editProfilUser(token,id,firstName,lastName,email,password,passwordConfirm);
            if(res.status === 200){
    ...
    return (
        ...
        <Button variant="gradient" fullWidth color="blue" onClick={() => handleProfilUser(token,id,firstName,lastName,email,password,passwordConfirm)}>
